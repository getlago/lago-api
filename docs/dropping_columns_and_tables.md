...
1. Create a dedicated migration commit that:
   ...
   - **Documents the version** that removed the references to the column/table and eventually introduced the `ignored_columns` entry
    ...
    The migration should use a rubocop disable comment:

    ```ruby
    class DropDeprecatedColumns < ActiveRecord::Migration[8.0]
      def change
        safety_assured do
          # rubocop:disable Lago/NoDropColumnOrTable
          remove_column :users, :deprecated_column_name
          remove_column :subscriptions, :old_status
