...
- **Best-effort latest state, not at-least-once.** Only one delivery per customer runs at a
  time, and a second one that overlaps it is dropped rather than queued behind it. Which
  snapshot survives depends on the timing:
  - A refresh arriving while an earlier delivery is still *queued* is rejected at enqueue. The
    queued job then reads at its own start time, so the delivered snapshot is the fresher one.
  - A refresh arriving while an earlier delivery is *running* is dropped instead. The running
    job wins, and the snapshot that goes out is the older of the two.

  A dropped snapshot is never redelivered. Normally the next wallet refresh supersedes it, so
  the staleness lasts one cycle.
- **The last snapshot before a customer goes idle can stay stale.** `awaiting_wallet_refresh`
  has already been cleared by the run that enqueued the dropped delivery, so a drop leaves
  nothing pending. If the customer generates no further usage, no later refresh corrects it and
  the consumer keeps a snapshot missing the usage between those two refreshes. The gap is
  bounded by how much usage lands in that window, which is seconds' worth, but it does not heal
  on its own. This is an accepted limit of best-effort delivery, not an oversight: closing it
  would mean re-enqueuing dropped deliveries, which adds retry pressure during exactly the
  bursts this path is built for.
