...
module GoldenLedger
  ...
  # The denominator as a recordable snapshot: every legal cell of every block, with the axis names
  # stated once per block so the recorded file stays scannable. `rake golden:baseline` writes this to
  # baseline.yml; denominator_delta reads it back.
  def denominator_snapshot
    blocks.to_h do |block|
      names = block["axes"].keys
      [block["id"], {"axes" => names, "cells" => legal_cells(block).map { |cell| cell.values_at(*names) }}]
    end
  end

  # The cells present now and absent in a recorded snapshot, grouped by block — the surveyor's
  # "what grew since the baseline". Cells are compared as {axis => value} hashes, rebuilt from each
  # side's own axis order, so a reordered axis list is not mistaken for new cells. A block the
  # snapshot has never seen counts as new in its entirety.
  def denominator_delta(against:)
    recorded = against.fetch("cells_by_block", {})

    denominator_snapshot.each_with_object({}) do |(block_id, current), out|
      before = recorded[block_id] || {"axes" => current["axes"], "cells" => []}
      before_cells = before["cells"].map { |values| before["axes"].zip(values.map(&:to_s)).to_h }
      now_cells = current["cells"].map { |values| current["axes"].zip(values).to_h }

      new_cells = now_cells - before_cells
      out[block_id] = new_cells unless new_cells.empty?
    end
  end
