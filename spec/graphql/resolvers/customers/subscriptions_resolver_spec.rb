...
RSpec.describe Resolvers::Customers::SubscriptionsResolver, type: :graphql do
  ...
  describe "when no status filter is provided" do
    ...
    let!(:downgraded_pending) { create(:subscription, customer:, plan:, status: "pending") }
    let(:active_from_downgrade) { create(:subscription, customer:, plan:, status: "active", previous_subscription: downgraded_pending) }
