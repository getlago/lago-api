...
RSpec.describe Resolvers::ProductItemFiltersResolver do
  ...
  let(:required_permission) { "product_item_filters:view" }
  ...
  let(:query) do
    <<~GQL
      query($searchTerm: String, $productItemId: ID, $productIds: [ID!], $withoutProduct: Boolean) {
        productItemFilters(limit: 5, searchTerm: $searchTerm, productItemId: $productItemId, productIds: $productIds, withoutProduct: $withoutProduct) {
  ...
  it_behaves_like "requires permission", "product_item_filters:view"
  ...
  context "with a product filter" do
    let(:variables) { {productIds: [product_item.product_id]} }
  ...
  context "with a without product filter" do
    ...
    it "returns only the filters of items not attached to any product" do
      expect(execution["data"]["productItemFilters"]["collection"].map { it["id"] }).to eq([orphan_filter.id])
    end
  end
