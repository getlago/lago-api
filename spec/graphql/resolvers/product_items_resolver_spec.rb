...
RSpec.describe Resolvers::ProductItemsResolver do
  ...
  let(:query) do
    <<~GQL
      query($searchTerm: String, $itemType: ProductItemTypeEnum, $productIds: [ID!], $withoutProduct: Boolean) {
        productItems(limit: 5, searchTerm: $searchTerm, itemType: $itemType, productIds: $productIds, withoutProduct: $withoutProduct) {
