...
RSpec.describe Mutations::Integrations::NetsuiteV2::Create do
  ...
  let(:mutation) do
    <<-GQL
      mutation($input: CreateNetsuiteV2IntegrationInput!) {
  ...
  before { membership.organization.update!(premium_integrations: ["netsuite_v2"]) }

  it_behaves_like "requires current user"
  it_behaves_like "requires current organization"
  it_behaves_like "requires permission", "organization:integrations:create"

  it "creates a netsuite V2 integration" do
    ...
    result_data = result["data"]["createNetsuiteV2Integration"]
