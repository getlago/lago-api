...
RSpec.describe Mutations::Subscriptions::DestroyChargeFilter, :premium do
  ...
  let(:query) do
    <<~GQL
      mutation($input: DestroySubscriptionChargeFilterInput!) {
        destroySubscriptionChargeFilter(input: $input) {
          id
        }
      }
    GQL
  ...
  it "creates a plan override and charge override, then destroys the filter" do
    expect { subject }
      .to change(Plan, :count).by(1)
      .and change(Charge, :count).by(1)

    result_data = subject["data"]["destroySubscriptionChargeFilter"]

    expect(result_data["id"]).to be_present
    destroyed_filter = ChargeFilter.unscoped.find(result_data["id"])
    expect(destroyed_filter).to be_discarded
  end
  ...
  context "when subscription already has plan override with charge and filter" do
    ...
    it "soft deletes the existing filter override" do
      ...
      expect(overridden_filter.reload).to be_discarded
    end
  end
end
