...
RSpec.describe Mutations::PlanRateCards::Create do
  ...
  let(:mutation) do
    <<~GQL
      mutation($input: CreatePlanRateCardInput!) {
        createPlanRateCard(input: $input) {
          id
          units
          ratePhasesCount
          productItem { id }
  ...
  it "assigns the product item to the plan with a default rate phase" do
    response = execution["data"]["createPlanRateCard"]

    expect(response["productItem"]["id"]).to eq(rate_card.product_item.id)
