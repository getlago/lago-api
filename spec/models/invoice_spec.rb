...
RSpec.describe Invoice, type: :model do
  describe 'validate_date_bounds' do
    let(:from_date) { Time.zone.now - 2.days }
    let(:to_date) { Time.zone.now }
    let(:invoice) do
      build(:invoice, from_date: from_date, to_date: to_date)
    end
    ...
    context 'when from_date and to_date are nil' do
      let(:from_date) { nil }
      let(:to_date) { nil }

      it 'invoice is valid' do
        expect(invoice).to be_valid
      end
    end
  end
