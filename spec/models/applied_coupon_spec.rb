...
RSpec.describe AppliedCoupon do
  ...
  describe "#remaining_amount_for_this_subscription_billing_period" do
    ...
    context "without any prior credits" do
      it "returns the full allowance without prior credits" do
    ...
    context "with a credit on another invoice in the same period" do
      ...
      it "deducts credits from the same billing period" do
    ...
    context "when called multiple times for the same invoice" do
      it "returns the same remaining allowance" do
    ...
    context "with multiple subscriptions on the invoice" do
      ...
      context "with a credit on the shared invoice" do
        ...
        it "counts a shared invoice credit only once" do
          expect(applied_coupon.remaining_amount_for_this_subscription_billing_period(invoice:)).to eq(80)
        end
      end
      ...
      context "with credits split between subscriptions" do
        ...
        it "sums distinct credits across all in-period invoices" do
          expect(applied_coupon.remaining_amount_for_this_subscription_billing_period(invoice:)).to eq(50)
        end
      end
