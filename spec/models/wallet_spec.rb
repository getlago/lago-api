...
RSpec.describe Wallet, type: :model do
  ...
  describe "#apply_top_up_limits" do
    subject { wallet.apply_top_up_limits(credit_amount:) }

    let(:wallet) { build_stubbed(:wallet, paid_top_up_min_amount_cents:, paid_top_up_max_amount_cents:) }

    context "when wallet has minimal top up limit specified" do
      let(:paid_top_up_min_amount_cents) { 10_00 }
      ...
      context "when wallet has maximum top up limit specified" do
        ...
        context "when provided value is greater than maximum" do
          ...
          it "returns max limit" do
            expect(subject).to eq 50
          end
        end

        context "when provided value is below than minimum" do
          let(:credit_amount) { 1 }

          it "returns min limit" do
            expect(subject).to eq 10
          end
        end

        context "when provided value is within the limits" do
      ...
      context "when wallet has maximum top up limit not specified" do
    ...
    context "when wallet has minimal top up limit not specified" do
