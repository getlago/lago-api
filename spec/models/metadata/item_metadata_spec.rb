  ...
  describe "validations" do
    ...
    describe "of owner uniqueness" do
      ...
      context "when owner is already taken" do
        ...
        it "adds an error" do
          expect(item_metadata).not_to be_valid
          expect(item_metadata.errors[:owner_id]).to be_present
        end
      end
    end
    ...
    describe "value must be JSON object constraint" do
      it "prevents non-object JSON values" do
        expect do
          ...
          SQL
        end.to raise_error(ActiveRecord::StatementInvalid, /metadata value must be a JSON object/)
      end
    end

    describe "trigger validation function" do
      context "when value has more than 50 keys" do
        let(:value) { 51.times.to_h { |i| ["key#{i}", "value#{i}"] } }

        it "raises database error" do
          expect { item_metadata.save!(validate: false) }
            .to raise_error(ActiveRecord::StatementInvalid, /metadata cannot have more than 50 keys/)
        end
      end
      ...
      context "when key is empty" do
        ...
        it "allows empty keys in database" do
          expect { item_metadata.save!(validate: false) }.not_to raise_error
        end
      end
      ...
      context "when key length is more than 40" do
        ...
        it "raises database error" do
          expect { item_metadata.save!(validate: false) }
            .to raise_error(ActiveRecord::StatementInvalid, /metadata key length cannot exceed 40 characters/)
        end
      end

      context "when value is not a string or null" do
        let(:value) { {"foo" => 123} }

        it "raises database error" do
          expect { item_metadata.save!(validate: false) }
            .to raise_error(ActiveRecord::StatementInvalid, /metadata values must be nulls or strings/)
        end
      end
      ...
      context "when value is null" do
        ...
        it "allows null values in database" do
          expect { item_metadata.save!(validate: false) }.not_to raise_error
        end
      end
      ...
      context "when value is empty string" do
        ...
        it "allows empty string values in database" do
          expect { item_metadata.save!(validate: false) }.not_to raise_error
        end
      end
      ...
      context "when value length is more than 255" do
        ...
        it "raises database error" do
          expect { item_metadata.save!(validate: false) }
            .to raise_error(ActiveRecord::StatementInvalid, /metadata value length cannot exceed 255 characters/)
        end
      end
    end
  end
