...
RSpec.describe Api::ForbidsLegacyBilling do
  ...
  describe "on a controller defining only a subset of the write actions" do
    # Hosts like Api::V1::Subscriptions::ChargesController define only a
    # subset of the write actions; the filter must not raise on missing
    # callback actions for them.
    # rubocop:disable RSpec/DescribedClass
    controller(ApplicationController) do
      include ApiErrors
      include Api::ForbidsLegacyBilling
