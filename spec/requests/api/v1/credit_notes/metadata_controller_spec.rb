...
RSpec.describe Api::V1::CreditNotes::MetadataController do
  ...
  describe "DELETE /api/v1/credit_notes/:id/metadata/:key" do
    ...
    context "when key exists in metadata" do
      before do
        metadata = create(:item_metadata, owner: credit_note, organization:, value: {"foo" => "bar", "baz" => "qux"})
        credit_note.update!(metadata_id: metadata.id)
      end
    ...
    context "when key does not exist in metadata" do
      before do
        metadata = create(:item_metadata, owner: credit_note, organization:, value: {"baz" => "qux"})
        credit_note.update!(metadata_id: metadata.id)
      end
