...
RSpec.describe Api::V1::ProductItemsController do
  ...
  describe "POST /api/v1/product_items" do
    ...
    it "creates a product item" do
      ...
      expect(json[:product_item][:product_code]).to eq(product.code)
      expect(json[:product_item][:billable_metric_code]).to eq(billable_metric.code)
    end
    ...
    context "with a standalone fixed item" do
      ...
      it "creates the item without product nor metric" do
        ...
        expect(json[:product_item][:product_code]).to be_nil
        expect(json[:product_item][:billable_metric_code]).to be_nil
      end
    end
  ...
  describe "GET /api/v1/product_items/:id" do
    ...
    it "returns the product item" do
      ...
      expect(json[:product_item][:code]).to eq(product_item.code)
      expect(json[:product_item][:filters_count]).to eq(1)
    end
  ...
  describe "GET /api/v1/product_items" do
    ...
    context "with multiple product_code values" do
      let(:other_product) { create(:product, organization:) }
      let!(:other_item) { create(:product_item, organization:, product: other_product) }
      let(:query_params) { "?product_code[]=#{product.code}&product_code[]=#{other_product.code}" }

      it "returns the items of all requested products" do
        subject

        expect(json[:product_items].map { it[:lago_id] }).to match_array([usage_item.id, other_item.id])
      end
    end

    context "with a without_product filter" do
      let(:query_params) { "?without_product=true" }
    ...
    context "with product_code and without_product combined" do
      let(:query_params) { "?product_code[]=#{product.code}&without_product=true" }
    ...
    context "when one of the product codes does not exist" do
      let(:query_params) { "?product_code[]=#{product.code}&product_code[]=unknown" }
