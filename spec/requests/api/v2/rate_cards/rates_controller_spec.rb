...
RSpec.describe Api::V2::RateCards::RatesController do
  ...
  describe "PUT /api/v2/rate_cards/:rate_card_id/rates/:id" do
    subject { put_with_token(organization, "/api/v2/rate_cards/#{rate_card.code}/rates/#{rate.id}", {rate: update_params}) }
    ...
    context "when the rate does not exist" do
      subject { put_with_token(organization, "/api/v2/rate_cards/#{rate_card.code}/rates/#{SecureRandom.uuid}", {rate: update_params}) }
  ...
  describe "GET /api/v2/rate_cards/:rate_card_id/rates/:id" do
    subject { get_with_token(organization, "/api/v2/rate_cards/#{rate_card.code}/rates/#{rate.id}") }
  ...
  describe "DELETE /api/v2/rate_cards/:rate_card_id/rates/:id" do
    subject { delete_with_token(organization, "/api/v2/rate_cards/#{rate_card.code}/rates/#{rate.id}") }
