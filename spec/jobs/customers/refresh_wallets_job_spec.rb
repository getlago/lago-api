...
RSpec.describe Customers::RefreshWalletsJob do
  ...
  describe "#perform" do
    ...
    let(:result) { Customers::RecomputeWalletsBalanceService::Result.new }
    let(:wallet_ids) { nil }

    before do
      allow(Customers::RecomputeWalletsBalanceService).to receive(:call).with(customer:).and_return(result)
    end
    ...
    [
      ...
      context "when the refresh fails with #{error_class.name.demodulize.underscore.humanize.downcase}" do
        before do
          allow(Customers::RecomputeWalletsBalanceService).to receive(:call).with(customer:).and_raise(error_class)
        end
    ...
    context "when the refresh is throttled by the tax provider" do
      ...
      before do
        allow(Customers::RecomputeWalletsBalanceService).to receive(:call).with(customer:).and_raise(error)
      end
