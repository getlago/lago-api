...
RSpec.describe DatabaseMigrations::BackfillMoneyhashPaymentMethodsJob do
  ...
  let(:moneyhash_customer) do
    create(:moneyhash_customer, customer:, payment_provider: moneyhash_provider).tap do |c|
      c.update!(payment_method_id: "mh_pm_123", provider_customer_id: "mh_customer_123")
    end
  end

  context "when moneyhash customer has a payment_method_id" do
    ...
    it "sets the correct attributes on the payment method" do
      ...
      expect(payment_method.provider_method_id).to eq("mh_pm_123")
      expect(payment_method.provider_method_type).to eq("card")
      expect(payment_method.payment_provider_customer).to eq(moneyhash_customer)
      expect(payment_method.payment_provider).to eq(moneyhash_provider)
      ...
      expect(payment_method.details["provider_customer_id"]).to eq("mh_customer_123")
    end
  end

  context "when moneyhash customer has no payment_method_id" do
  ...
  context "when payment method already exists for the moneyhash customer" do
    it "does not create a duplicate" do
      create(:payment_method, customer:, payment_provider_customer: moneyhash_customer, provider_method_id: "mh_pm_123")

      expect { perform_job }.not_to change(PaymentMethod, :count)
    end
  end

  context "when a discarded payment method already exists for the moneyhash customer" do
    it "does not recreate the discarded payment method" do
      create(:payment_method, customer:, payment_provider_customer: moneyhash_customer, provider_method_id: "mh_pm_123").discard!
  ...
  context "with organization_id filter" do
    ...
    let(:other_moneyhash_provider) { create(:moneyhash_provider, organization: other_organization) }
    let(:other_moneyhash_customer) do
      create(:moneyhash_customer, customer: other_customer, payment_provider: other_moneyhash_provider).tap do |c|
        c.update!(payment_method_id: "mh_pm_other")
      end
    end

    it "only processes the specified organization" do
      moneyhash_customer
      other_moneyhash_customer
  ...
  context "when there is more work after the batch" do
    ...
    it "enqueues the next batch" do
      ...
      moneyhash_customer
      create(:moneyhash_customer, customer: customer_2, payment_provider: moneyhash_provider).tap do |c|
        c.update!(payment_method_id: "mh_pm_456")
      end
