...
1. **Coverage is computed, not guessed.** The suite knows how many cells exist, how many are covered,
   and which are missing — see [COVERAGE.md](COVERAGE.md), regenerated by `rake golden:docs`.
...
| `COVERAGE.md` | generated | the summary — **do not edit by hand** |
...
| `../../../lib/tasks/golden.rake` | code | `golden:ledger`, `golden:docs`, `golden:surface`, `golden:discover`, `golden:state` |
...
- **`CHANGELOG.md`** — created with the first expectation change.
...
- **Rows.** 79 of 870 cells, **all of which are writeable** — the capability backlog is clear, so
  every uncovered cell is a row somebody can sit down and write. 15 of 60 actions. **B14 is complete at 25/25**; Eleven of sixteen blocks now have rows
  (B1 12/70, B3 10/48, B4 3/16, B5 3/27, B6 7/144, B7 4/96, B8 4/48, B9 3/48, B11 3/21, B2 1/40). See [COVERAGE.md](COVERAGE.md) for the
  ranked gaps; the remaining order is by RISK — B6 wallets, B8 credit notes, B7 usage — with B15 last
  because it needs the other blocks to exist.
