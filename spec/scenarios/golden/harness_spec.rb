...
describe "Golden harness" do
  ...
  describe "generated inventory is current" do
    # scenarios.json is a pure function of the rows and blocks, so a stale one is detectable rather
    # than a thing people remember to refresh. It is checked and COVERAGE.md is not, because COVERAGE
    # also depends on bugmap.json — which is generated on the host and legitimately lags.
    it "matches the rows on disk" do
      committed = JSON.parse(File.read(GoldenMatrix.dir.join("scenarios.json")))
      current = JSON.parse(GoldenLedger.scenario_summary.to_json)

      expect(current).to eq(committed),
        "spec/scenarios/golden/scenarios.json is stale — #{current["generated_from_rows"]} rows on " \
        "disk vs #{committed["generated_from_rows"]} in the file. Run `rake golden:docs`. An inventory " \
        "that lags the matrix describes scenarios the suite may no longer run."
    end
  end
