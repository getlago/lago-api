...
describe "Pay in advance charges Scenarios", transaction: false do
  [:postgres, :clickhouse].each do |store|
    context "with #{store} store", clickhouse: store == :clickhouse do
      ...
      describe "with sum_agg / dynamic" do
        ...
        it "creates an pay_in_advance fee" do
          ...
          # Create events with timestamp AFTER the current event to invoice, to ensure `event.timestamp` is used as upper bound.
          # It creates differences in the aggregation but not in the fees created
          create(:event, code: billable_metric.code, organization:, subscription:, timestamp: Time.zone.parse("2024-09-30 11:00"), properties: {amount: 9876}, precise_total_amount_cents: "77778888.9999")
