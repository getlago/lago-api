...
# Vendored verbatim from app/services/billing_periods/boundaries.rb
# Do not edit: this is the frozen pre-rewrite engine.
module LegacyEngine; end

module LegacyEngine::BillingPeriods
  ...
  class Boundaries
    ...
    # too low). The `if` catches the "one too high" case and steps back one boundary
    # when the guessed boundary lands after `time`. Indices go negative: the anchor is
    # just a reference day, and a `time` before it falls in an earlier period (index < 0),
    # so a subscription that starts before its anchor day still bills from its start.
    #
    ...
    # Example, anchor Jul 15, index_on_or_before(Jul 2):
    #   estimate = 0, but at(0) = Jul 15 > Jul 2 => overshot => return -1
    #   (Jul 2 is in period -1: [Jun 15, Jul 15))
    def index_on_or_before(time)
      estimate = estimated_index(time)
      return estimate - 1 if at(estimate) > time
    ...
    def billed_days(from, to)
      LegacyEngine::Utils::Datetime.date_diff_with_timezone(from, to, timezone)
    end
    ...
    #
    # Can be negative for a `time` before the anchor (integer division floors toward
    # negative infinity, which keeps the "exact or one too high, never too low" property).
