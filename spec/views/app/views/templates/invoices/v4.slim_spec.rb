...
RSpec.describe "templates/invoices/v4.slim" do
  ...
  context "when invoice has pay-in-advance charges with prepaid credits" do
    let(:organization) { create(:organization, :with_static_values) }
    let(:customer) { create(:customer, :with_static_values, organization:) }
    ...
    let(:plan) do
      create(
        ...
        interval: "monthly",
        pay_in_advance: false,
        invoice_display_name: "Plan with Pay-in-Advance Charges"
      )
    end
    ...
    let(:wallet) { create(:wallet, customer:, organization:, rate_amount: BigDecimal("1.0"), balance_currency: "USD") }
    let(:wallet_transaction) { create(:wallet_transaction, wallet:, invoice:, credit_amount: BigDecimal("8.0"), amount: BigDecimal("8.0")) }

    let(:invoice) do
      create(
        :invoice,
        customer:,
        number: "LAGO-202509-007",
        payment_due_date: Date.parse("2025-09-19"),
        issuing_date: Date.parse("2025-09-04"),
        invoice_type: :subscription,
        ...
        sub_total_including_taxes_amount_cents: 7000,
        prepaid_credit_amount_cents: 800,
        prepaid_granted_credit_amount_cents: 300,
        prepaid_purchased_credit_amount_cents: 500
      )
    end
    ...
    let(:pay_in_advance_fixed_charge) do
      create(:fixed_charge, :pay_in_advance, plan:, add_on:)
    end

    let(:pay_in_advance_fixed_charge_fee) do
      create(
        :fixed_charge_fee,
        invoice:,
        fixed_charge: pay_in_advance_fixed_charge,
        subscription:,
        pay_in_advance: true,
        amount_cents: 5000,
        amount_currency: "USD",
        ...
        unit_amount_cents: 2500,
        precise_unit_amount: 25.00,
        invoice_display_name: "Pay-in-Advance Fixed Charge Fee",
        properties: {
          fixed_charges_from_datetime: "2025-09-01 00:00:00",
          fixed_charges_to_datetime: "2025-09-30 23:59:59"
        }
    ...
    before do
      invoice_subscription
      subscription_fee
      pay_in_advance_fixed_charge_fee
      wallet_transaction
    end
