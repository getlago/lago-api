...
RSpec.describe CreditNotes::ValidateService do
  ...
  describe ".call" do
    ...
    context "when invoice is type credit with non-traceable wallet" do
      let(:organization) { create(:organization) }
      let(:cust) { create(:customer, organization:) }
      let(:wallet) { create(:wallet, customer: cust, balance_cents: 500, traceable: false) }
      let(:inbound_wallet_transaction) do
        create(:wallet_transaction,
          wallet:,
          organization:,
      ...
      it "fails validation when refund exceeds wallet balance" do
        expect(validator).not_to be_valid
