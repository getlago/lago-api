...
RSpec.describe Integrations::Aggregator::Taxes::CreditNotes::Payloads::Anrok do
  describe "#body" do
    ...
    context "when a charge is credited over several fees" do
      ...
      it "credits each charge as a single line item with a distinct identifier" do
        expect(payload.first["fees"]).to eq(
          [
            {"item_id" => charge.id, "item_code" => "ext_123", "amount_cents" => -1000},
            {"item_id" => other_charge.id, "item_code" => "ext_123", "amount_cents" => -500}
          ]
        )
      end
    end
    ...
    context "with precision edge case" do
      ...
      before do
        ...
        allow(items_relation).to receive(:order).with(created_at: :asc, id: :asc).and_return([credit_note_item])
      end
