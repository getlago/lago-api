  ...
  describe ".call" do
    ...
    context "with a single-day range overlapping an advance period" do
      ...
      it "schedules the advance period due for the item" do
        ...
        expect(billing_segment.period_to).to eq(Time.zone.parse("2026-08-31 23:59:59.999999"))
        expect(billing_segment.billing_at).to eq(current_time)
    ...
    context "without an explicit range" do
      ...
      context "when the generated advance period starts in the past" do
        ...
        it "sets billing_at to the scheduling time" do
          ...
          billing_segment = result.billing_segments.sole
          expect(billing_segment.period_from).to eq(Time.zone.parse("2026-08-10"))
          expect(billing_segment.period_to).to eq(Time.zone.parse("2026-08-19 23:59:59.999999"))
          expect(billing_segment.billing_at).to eq(current_time)
        end
      end
    end
    ...
    context "when the generated advance period starts in the future" do
      ...
      it "keeps billing_at on the future billing boundary" do
        ...
        billing_segment = result.billing_segments.sole
        expect(billing_segment.period_from).to eq(Time.zone.parse("2026-08-20"))
        expect(billing_segment.period_to).to eq(Time.zone.parse("2026-09-19 23:59:59.999999"))
        expect(billing_segment.billing_at).to eq(Time.zone.parse("2026-08-20"))
      end
    end
    ...
    context "with a closed date range" do
      ...
      it "schedules billing segments overlapping the range" do
        expect { result }.to change(BillingSegment, :count).by(1)

        billing_segment = result.billing_segments.sole
        expect(billing_segment.period_from).to eq(Time.zone.parse("2026-07-01"))
        expect(billing_segment.period_to).to eq(Time.zone.parse("2026-07-31 23:59:59.999999"))
        expect(billing_segment.billing_at).to eq(current_time)
        expect(billing_segment.rate_card_rate).to eq(rate_card_rate)
        expect(billing_segment.rate_override).to be_nil
        expect(customer.subscription_rate_cards.sole.reload.next_billing_at).to eq(Time.zone.parse("2026-09-01"))
      end
      ...
      context "with a rate card pricing unit" do
        ...
        it "stores the pricing unit on the billing segment" do
          expect { result }.to change(BillingSegment, :count).by(1)
    ...
    context "with rate changes inside the range" do
      ...
      it "splits billing segments across rate effective dates and calendar periods" do
        expect { result }.to change(BillingSegment, :count).by(2)
        ...
        expect(result.billing_segments.map(&:rate_card_rate)).to eq([rate_card_rate, second_rate_card_rate])
      end
    end
    ...
    context "with a phase rate override" do
      ...
      it "stores the base rate and override on the billing segment" do
        expect { result }.to change(BillingSegment, :count).by(1)

        billing_segment = result.billing_segments.sole
        expect(billing_segment.rate_card_rate).to eq(rate_card_rate)
        expect(billing_segment.rate_override).to eq(rate_override)
    ...
    context "with a rate change in the middle of a weekly period" do
      ...
      it "splits the billing period at the rate effective date" do
        ...
        expect(result.billing_segments.map(&:proration_ratio)).to eq([1, 1, 1])
      end
      ...
      context "with proration enabled" do
        ...
        it "stores the fee proration ratio for each split period" do
          ...
          expect(result.billing_segments.map(&:proration_ratio)).to eq([
            BigDecimal("1"),
      ...
      context "when an earlier out-of-range period is already persisted" do
        ...
        it "skips the out-of-range period and schedules the overlapping ones" do
          expect { result }.to change(BillingSegment, :count).by(6)

          expect(result).to be_success
          expect(result.billing_segments.map(&:period_from)).not_to include(Time.zone.parse("2026-07-27"))
          expect(result.billing_segments.first.period_from).to eq(Time.zone.parse("2026-08-03"))
        end
      end
    end
    ...
    context "when a single-day range includes the next billing date" do
      ...
      context "with a two-day lookback range" do
        ...
        it "schedules the closed arrears period overlapping the range" do
          ...
          expect(billing_segment.billing_at).to eq(current_time)
          expect(billing_segment.subscription_rate_card.reload.next_billing_at).to eq(Time.zone.parse("2026-10-10"))
        end
        ...
        context "with a customer timezone" do
          ...
          it "floors the subscription rate card start in the customer timezone" do
            ...
            billing_segment = result.billing_segments.sole
            expect(billing_segment.period_from).to eq(Time.zone.parse("2026-08-09 22:00:00"))
            expect(billing_segment.period_to).to eq(Time.zone.parse("2026-09-09 21:59:59.999999"))
            expect(billing_segment.subscription_rate_card.reload.next_billing_at).to eq(Time.zone.parse("2026-10-09 22:00:00"))
          end
        end
      end
      ...
      context "with an advance rate card" do
        ...
        it "schedules the full advance period due on the requested day" do
          ...
          billing_segment = result.billing_segments.sole
          expect(billing_segment.period_from).to eq(Time.zone.parse("2026-09-10"))
          expect(billing_segment.period_to).to eq(Time.zone.parse("2026-10-09 23:59:59.999999"))
          expect(billing_segment.billing_at).to eq(current_time)
          expect(billing_segment.subscription_rate_card.reload.next_billing_at).to eq(Time.zone.parse("2026-10-10"))
        end
      end
    ...
    context "when different subscription rate cards have overlapping periods" do
      ...
      it "schedules every billing segment" do
        expect { result }.to change(BillingSegment, :count).by(2)

        expect(result).to be_success
        expect(result.billing_segments.map(&:subscription_rate_card_id).uniq.count).to eq(2)
      end
    end

    context "when a persisted billing segment overlaps the generated period" do
      let(:range) { "2026-07-15".."2026-08-14" }
      ...
      it "returns an error and does not persist the generated billing segment" do
        expect { result }.not_to change(BillingSegment, :count)
    ...
    context "when the generated billing segment already exists" do
      let(:range) { "2026-07-15".."2026-08-14" }
      ...
      it "returns an error and does not persist a duplicate billing segment" do
        expect { result }.not_to change(BillingSegment, :count)
