...
RSpec.describe BillingSegments::ScheduleAdvanceIncrementService do
  ...
  it "schedules the remaining window with the share of the period still ahead" do
    expect(result.billing_segment).to be_present
    expect(result.billing_segment.period_from).to eq(at)
    expect(result.billing_segment.period_to).to be_within(1.second).of(period_to)
    expect(result.billing_segment.proration_ratio).to eq((BigDecimal(30) / 31).round(10))
    expect(result.billing_segment.subscription_rate_card).to eq(card)
  end

  it "prices to $60.00 through the ordinary path" do
    fee = BillingSegments::Fees::ComputeService.call!(billing_segment: result.billing_segment).fee
  ...
  context "when a rate change split the period" do
    ...
    it "schedules the rest of that segment only" do
      expect(result.billing_segment.period_to).to be_within(1.second).of(segment_end)
      expect(result.billing_segment.proration_ratio).to eq((BigDecimal(12) / 31).round(10))
    end
  end
end
