...
RSpec.describe Events::Stores::Provider do
  subject(:provider) { described_class.new(organization:, context:, boundaries:) }
  ...
  describe "#store_for" do
    it "returns a store configured for the metered item and the computation window" do
      store = provider.store_for(metered_item:)
    ...
    it "forwards the filters" do
      ...
      store = provider.store_for(metered_item:, filters:)
    ...
    it "mints one instance per call, so aggregators cannot share per-charge state" do
      first = provider.store_for(metered_item:)
      second = provider.store_for(metered_item:)
