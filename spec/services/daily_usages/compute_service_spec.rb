...
RSpec.describe DailyUsages::ComputeService do
  ...
  let(:subscription) do
    create(:subscription, :calendar, customer:, plan:, started_at: timestamp - 1.year, subscription_at: timestamp - 1.year, renew_daily_usage: true)
  end
  ...
  describe "#call" do
    ...
    context "when there is usage" do
      ...
      context "when timestamp is on subscription billing day" do
        let(:subscription) do
          create(:subscription, :anniversary, customer:, plan:, started_at: 1.year.ago, subscription_at: 1.year.ago, renew_daily_usage: true)
        end
