...
RSpec.describe Roles::UpdateService do
  describe "#call" do
    ...
    context "when name is changed and pending invites exist" do
      let!(:invite_with_role) { create(:invite, organization:, roles: ["Old Name"], status: :pending) }
      let!(:invite_with_other_role) { create(:invite, organization:, roles: ["Other Role"], status: :pending) }
      let!(:accepted_invite) { create(:invite, organization:, roles: ["Old Name"], status: :accepted) }

      it "updates pending invites with the old role name" do
        expect { result }.to change { invite_with_role.reload.roles }.from(["Old Name"]).to(["New Name"])
      end

      it "does not update pending invites with other role names" do
