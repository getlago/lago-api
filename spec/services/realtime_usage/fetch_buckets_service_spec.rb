...
RSpec.describe RealtimeUsage::FetchBucketsService, clickhouse: {clean_before: true} do
  ...
  describe "#call" do
    ...
    context "when clickhouse is unreachable" do
      ...
      it "fails, so the caller knows the buckets were not read rather than empty" do
        expect(fetch).to be_failure
        expect(fetch.error).to be_a(BaseService::ServiceFailure)
        expect(fetch.error.code).to eq("usage_buckets_read_failure")
        expect(fetch.usage_buckets).to be_nil
      end
    end

    # The driver signals a broken connection with any of these, and none of them is an
    # ActiveRecordError, so each one used to reach the caller as a 500.
    [Errno::ECONNRESET, NoMethodError, Net::ReadTimeout, JSON::ParserError].each do |error_class|
      context "when the read raises #{error_class}" do
        before do
          allow(Clickhouse::UsageBucket).to receive(:where).and_raise(error_class)
          allow(Sentry).to receive(:capture_exception)
        end
        ...
        it "reports it, as a silent fallback would hide an unreachable clickhouse" do
          fetch

          expect(Sentry).to have_received(:capture_exception).with(an_instance_of(error_class))
        end
      end
    end

    context "when the read succeeds" do
      before do
        allow(Events::Stores::Utils::ClickhouseConnection).to receive(:with_retry).and_call_original
        create_bucket(bucket: from_datetime)
      end

      it "goes through the shared retry helper, like every other clickhouse read" do
