...
RSpec.describe BillingPeriods::Dates::ArrearsService do
  describe ".call" do
    subject(:result) do
    ...
    let(:range) { Date.parse("2022-03-01")..Date.parse("2022-03-01") }
    ...
    it "returns the period that closed at the billing boundary" do
      ...
      expect(result.periods.map { [it.period_from, it.period_to, it.next_billing_at, it.rate] }).to eq(
        [
          [
            Time.zone.parse("2022-02-01"),
            end_of_day.call("2022-02-28"),
    ...
    context "with different rate effective dates" do
      ...
      it "splits shifted periods at billing boundaries and rate effective dates" do
        ...
        expect(result.periods.map { [it.period_from, it.period_to, it.rate] }).to eq(
          [
            [Time.zone.parse("2026-07-20"), end_of_day.call("2026-07-26"), rate],
      ...
      context "when excluding periods outside the range" do
        let(:range) { Date.parse("2026-08-03")..Date.parse("2026-09-08") }
        let(:exclude_out_of_range) { true }

        it "skips fully out-of-range periods while preserving the generated next billing boundary" do
          expect(result.next_billing_at).to eq(Time.zone.parse("2026-09-14"))
          expect(result.periods.map { [it.period_from, it.period_to, it.rate] }).to eq(
            [
              [Time.zone.parse("2026-08-03"), Time.zone.parse("2026-08-05 23:59:59.999999"), rate],
              [Time.zone.parse("2026-08-06"), end_of_day.call("2026-08-09"), second_rate],
              [Time.zone.parse("2026-08-10"), end_of_day.call("2026-08-16"), second_rate],
              [Time.zone.parse("2026-08-17"), end_of_day.call("2026-08-23"), second_rate],
              [Time.zone.parse("2026-08-24"), end_of_day.call("2026-08-30"), second_rate],
              [Time.zone.parse("2026-08-31"), end_of_day.call("2026-09-06"), second_rate]
            ]
          )
        end
      end
    ...
    context "with three rates using different billing intervals" do
      ...
      let(:range) { Date.parse("2026-01-01")..Date.parse("2026-06-01") }
      ...
      it "uses each rate interval while splitting shifted periods on effective dates" do
        expect(result.next_billing_at).to eq(Time.zone.parse("2026-07-01"))
        expect(result.periods.map { [it.period_from, it.period_to, it.rate] }).to eq(
          [
