...
RSpec.describe Subscriptions::ProgressiveBilledAmount do
  ...
  context "with progressive billing invoice that has 100% coupon discount" do
    ...
    it "returns to_credit_amount equal to gross fees, transferring the coupon forward" do
      # ISSUE-1007: to_credit_amount no longer subtracts coupons_amount_cents. The gross fee
      # amount is credited on the next invoice so the coupon's discount survives the PB.
      result = service.call
      expect(result.progressive_billing_invoice).to eq(invoice)
  ...
  context "with progressive billing invoice that has partial coupon discount" do
    ...
    it "returns gross fees regardless of coupons" do
      # ISSUE-1007: to_credit_amount = gross fees, not net of coupons.
      result = service.call
      expect(result.progressive_billing_invoice).to eq(invoice)
      expect(result.progressive_billed_amount).to eq(100)
      expect(result.to_credit_amount).to eq(100)
  ...
  context "with multiple progressive billing invoices with coupons" do
    ...
    it "returns gross fees of the most recent PB invoice" do
  ...
  context "with progressive billing invoice with coupons and existing credits" do
    ...
    it "subtracts existing PB credits from gross fees" do
      # ISSUE-1007: 100 (gross fees) - 30 (existing PB credit) = 70. Coupons no longer subtracted.
      result = service.call
      expect(result.progressive_billing_invoice).to eq(invoice)
      expect(result.progressive_billed_amount).to eq(100)
  ...
  context "with progressive billing invoice with coupons and existing credit notes" do
    ...
    it "subtracts existing credit notes from gross fees" do
      # ISSUE-1007: 100 (gross fees) - 20 (credit note) = 80. Coupons no longer subtracted.
      result = service.call
      expect(result.progressive_billing_invoice).to eq(invoice)
      expect(result.progressive_billed_amount).to eq(100)
  ...
  context "with progressive billing invoice that has multiple fees with varying coupons" do
    ...
    it "correctly calculates total_billed_amount_cents from multiple fees with varying coupons" do
      # ISSUE-1007: to_credit_amount = gross fees (180), not net of coupons.
      result = service.call
      expect(result.progressive_billing_invoice).to eq(invoice)
