...
RSpec.describe Subscriptions::BillingPeriods::LockService do
  let(:lock_service) { described_class.new(subscription:, timeout_seconds:) }
  let(:subscription) { create(:subscription) }
  let(:timeout_seconds) { 5.seconds }
  let(:lock_key) { "subscription-billing-periods-#{subscription.id}" }
  ...
  describe "#call" do
    context "when lock can be acquired" do
      ...
      it "allows nesting a lock on another subscription" do
        other_subscription = create(:subscription)
        inner_ran = false

        lock_service.call do
          described_class.call(subscription: other_subscription) { inner_ran = true }
        end
