...
RSpec.describe Customers::RefreshWalletsService do
  ...
  let(:options) { {} }
  let(:refresh_result) { Customers::RecomputeWalletsBalanceService::Result.new }

  before do
    allow(Customers::RecomputeWalletsBalanceService).to receive(:call).with(customer:).and_return(refresh_result)
  end
  ...
  describe "#call" do
    it "refreshes the customer's wallets" do
      ...
      expect(Customers::RecomputeWalletsBalanceService).to have_received(:call).with(customer:)
    end
    ...
    context "when the customer is not awaiting a wallet refresh" do
      ...
      it "does not refresh" do
        ...
        expect(Customers::RecomputeWalletsBalanceService).not_to have_received(:call)
      end
    ...
    context "when the refresh fails with a tax error on the customer's address" do
      let(:refresh_result) do
        Customers::RecomputeWalletsBalanceService::Result.new.validation_failure!(errors: {tax_error: ["customerAddressCouldNotResolve"]})
      end
    ...
    context "when the refresh fails with an unknown tax error" do
      let(:refresh_result) do
        Customers::RecomputeWalletsBalanceService::Result.new.validation_failure!(errors: {tax_error: ["failure"]})
      end
    ...
    context "when the refresh fails with a non-tax error" do
      let(:refresh_result) do
        Customers::RecomputeWalletsBalanceService::Result.new.validation_failure!(errors: {other_error: ["something"]})
      end
