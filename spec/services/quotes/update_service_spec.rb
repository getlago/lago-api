      ...
      let(:params) do
        {
          billing_items: {
            "plans" => [
              {"id" => SecureRandom.uuid, "position" => 1, "plan_id" => SecureRandom.uuid, "plan_name" => "New Plan"}
            ],
            "coupons" => [],
            "wallet_credits" => []
          }
    ...
    context "when clearing billing_items" do
      let(:quote) do
        create(:quote, :subscription_creation, customer:, organization:, billing_items: {
          "plans" => [{"id" => SecureRandom.uuid, "position" => 1, "plan_id" => SecureRandom.uuid, "plan_name" => "Plan"}],
          "coupons" => [],
          "wallet_credits" => []
        })
      end
