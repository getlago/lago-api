...
RSpec.describe Plans::UpdateUsageThresholdsService, premium: true do
      ...
      context "when #{tc[:description]}" do
        ...
        before do
          tc[:existing].each do |attrs|
            create(:usage_threshold, plan:, organization:, threshold_display_name: nil, **attrs)
          end
        end

        it do
          expect(result).to be_success
          expect(threshold_attrs(result.plan.usage_thresholds)).to match_array(tc[:expected])
        end
      end
    end
  end
