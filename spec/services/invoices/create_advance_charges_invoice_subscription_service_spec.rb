...
RSpec.describe Invoices::CreateAdvanceChargesInvoiceSubscriptionService do
  ...
  describe "#call" do
    ...
    it "computes the boundaries on the current billing period" do
      ...
      expect(invoice_subscription.from_datetime).to match_datetime DateTime.new(2025, 1, 1)
      expect(invoice_subscription.to_datetime.to_date).to eq Date.new(2025, 1, 31)
    ...
    context "when every subscription is terminated at the billing timestamp" do
      # The termination flow bills the non-invoiceable fees on the termination date.
      let(:timestamp) { DateTime.new(2024, 4, 5, 10) }
      ...
      let(:all_subscriptions) { [previous_subscription, terminated_subscription] }

      it "computes the boundaries from the latest terminated subscription" do
        result = create_service.call
        ...
        expect(invoice_subscription.charges_to_datetime).to match_datetime terminated_subscription.terminated_at
      end
    end
  end
end
