...
RSpec.describe LifetimeUsages::CalculateService do
  ...
  describe "pay-in-advance usage already invoiced in the open period" do
    ...
    context "when the fee predates the current charge period" do
      let(:fee_created_at) { timestamp - 2.months }

      it "does not inspect or subtract the old fee" do
        result = service.call
