...
RSpec.describe WalletTransactions::TrackConsumptionService do
  ...
  describe "#call" do
    subject(:result) { described_class.call(outbound_wallet_transaction:, **args) }

    let(:args) { {} }

    context "when consuming by priority (no specific inbound)" do
      ...
      it "consumes granted credits first" do
        ...
        consumptions = outbound_wallet_transaction.fundings.order(:amount_cents_consumed)
        expect(consumptions.first.inbound_wallet_transaction).to eq(inbound_purchased)
        expect(consumptions.first.amount_cents_consumed).to eq(2000)

        expect(consumptions.second.inbound_wallet_transaction).to eq(inbound_granted)
        expect(consumptions.second.amount_cents_consumed).to eq(3000)
      end
    ...
    context "when consuming from a specific inbound transaction" do
      ...
      let(:args) { {inbound_wallet_transaction_id: inbound_transaction.id} }
      ...
      it "consumes from the specific inbound transaction" do
        ...
        consumption = outbound_wallet_transaction.fundings.first
        expect(consumption.inbound_wallet_transaction).to eq(inbound_transaction)
        expect(consumption.amount_cents_consumed).to eq(4000)
      end

      it "decrements remaining_amount_cents on the inbound transaction" do
        result

        expect(inbound_transaction.reload.remaining_amount_cents).to eq(6000)
      end
    ...
    context "when consuming from specific inbound with insufficient amount" do
      let!(:inbound_transaction) do
        create(:wallet_transaction,
          wallet:,
          organization:,
