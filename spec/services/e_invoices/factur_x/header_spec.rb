...
RSpec.describe EInvoices::FacturX::Header, type: :service do
  subject do
  ...
  describe ".call" do
    ...
    context "when invoice" do
      ...
      it "expects to have a type code" do
        expect(subject).to contains_xml_node("#{root}/ram:TypeCode").with_value(EInvoices::BaseService::COMMERCIAL_INVOICE)
      end
      ...
      it "expects to have invoice issuing date" do
        expect(subject).to contains_xml_node("#{root}/ram:IssueDateTime/udt:DateTimeString")
          ...
          .with_attribute("format", EInvoices::FacturX::BaseService::CCYYMMDD)
      end
    ...
    context "when credit note" do
      ...
      let(:type_code) { EInvoices::BaseService::CREDIT_NOTE }
      ...
      it "expects to have a type code" do
        expect(subject).to contains_xml_node("#{root}/ram:TypeCode").with_value(EInvoices::BaseService::CREDIT_NOTE)
      end
    ...
    context "when payment" do
      ...
      let(:type_code) { EInvoices::BaseService::PAYMENT_RECEIPT }
      ...
      it "expects to have a type code" do
        expect(subject).to contains_xml_node("#{root}/ram:TypeCode").with_value(EInvoices::BaseService::PAYMENT_RECEIPT)
      end
