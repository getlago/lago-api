...
RSpec.describe EInvoices::FacturX::ApplicableTradeTax, type: :service do
  subject do
    xml_document(:factur_x) do |xml|
      described_class.call(xml:, tax_category:, tax_rate:, basis_amount:, tax_amount:)
    end
  end

  let(:tax_category) { EInvoices::BaseService::S_CATEGORY }
  let(:tax_rate) { 20.00 }
  let(:basis_amount) { 10 }
  ...
  describe ".call" do
    ...
    it "has the category code" do
      expect(subject).to contains_xml_node("#{root}/ram:CategoryCode").with_value(EInvoices::BaseService::S_CATEGORY)
    end
    ...
    context "when category code is O" do
      let(:tax_category) { EInvoices::BaseService::O_CATEGORY }

      it "has ExemptionReasonCode" do
        expect(subject).to contains_xml_node("#{root}/ram:ExemptionReasonCode").with_value(EInvoices::BaseService::O_VAT_EXEMPTION)
      end
