...
class CreateEventsEnrichedExtendedQueue < ActiveRecord::Migration[8.0]
  def change
    options = <<-SQL
      Kafka()
      SETTINGS
        kafka_broker_list = '#{ENV["LAGO_KAFKA_BOOTSTRAP_SERVERS"]}',
        kafka_topic_list = '#{ENV["LAGO_KAFKA_ENRICHED_EVENTS_EXTENDED_TOPIC"]}',
        kafka_group_name = '#{ENV["LAGO_KAFKA_CLICKHOUSE_CONSUMER_GROUP"]}',
        kafka_format = 'JSONEachRow';
    SQL

    create_table :events_enriched_extended_queue, id: false, options: do |t|
