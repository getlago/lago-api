...
class DropEventsEnrichedExpanded < ActiveRecord::Migration[8.0]
  def up
    safety_assured do
      execute "DROP TABLE IF EXISTS events_enriched_expanded"
    end
  end
  ...
  def down
    ...
    create_table :events_enriched_expanded, id: false, options: do |t|
      ...
      t.datetime :enriched_at, null: false, precision: 3, default: -> { "now64(3)" }
