...
class CreateUsageBuckets15m < ActiveRecord::Migration[8.0]
  # 15 minutes is the granularity that makes every timezone's day boundary land on a bucket
  # wall, since every real UTC offset is a multiple of 15 minutes.
  #
  # Partitioned by month of the bucket so merges never range over the whole history and
  # retention is a DROP PARTITION rather than a TTL mutation.
  def up
    safety_assured do
      execute <<~SQL
