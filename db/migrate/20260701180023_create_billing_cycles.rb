...
class CreateBillingCycles < ActiveRecord::Migration[8.0]
  def change
    ...
    create_table :billing_cycles, id: :uuid do |t|
      ...
      # The invoice this cycle was billed into (nil until processed). Records the
      # outbox result and lets us re-enqueue the cycle (status -> pending, invoice_id
      # -> nil) when that invoice is voided/deleted/retried.
      ...
      # Idempotency backstop: one cycle per (product, period). A re-pickup is a
      # no-op insert, never a double bill.
      t.index [:subscription_rate_card_id, :period_from],
        unique: true,
        name: "index_billing_cycles_on_product_and_period"
