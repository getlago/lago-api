...
class FixStaleBillingEntitySequentialIdToBeUniq < ActiveRecord::Migration[8.0]
  ...
  def up
    # see BillingEntities::ChangeInvoiceNumberingService -- if we switch from per_customer 
    ...
    BillingEntity.where(document_numbering: 'per_billing_entity').find_each do |billing_entity|
      ...
      duplicates.each do |billing_entity_sequential_id|
        raise "DUPLICATED LATEST BILLING_ENTITY_SEQUENTIAL_ID: #{billing_entity_sequential_id}" if billing_entity_sequential_id >= existing_max_number

        Invoice.where(billing_entity_id: billing_entity.id, billing_entity_sequential_id: billing_entity_sequential_id)
          .update_all("billing_entity_sequential_id = NULL")
      end
    end
  end
