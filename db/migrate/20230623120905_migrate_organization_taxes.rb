      ...
      dir.up do
        ...
        sql = <<-SQL
          ...
          FROM existing_fees_taxes
            INNER JOIN fees ON existing_fees_taxes.fee_id = fees.id
        ...
        indexed_rows = ApplicationRecord.connection.select_all(sql).rows.each_with_object({}) do |row, result|
