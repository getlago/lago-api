...
class CreateEntitlementSubscriptionEntitlementsView < ActiveRecord::Migration[8.0]
  def up
    safety_assured do
      execute <<~SQL
        CREATE OR REPLACE VIEW entitlement_subscription_entitlements_view AS
        WITH
  ...
  def down
    safety_assured do
      execute "DROP VIEW entitlement_subscription_entitlements_view"
    end
  end
end
