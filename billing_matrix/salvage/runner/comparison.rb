...
module GoldenComparison
  ...
  # flat names. The mapping serves only payloads that nest: one that carries the field FLAT — the
  # charge resource itself carries charge_model at the top level — is already the answer, and
  # `read` prefers it. Applying a path unconditionally is how 31 charge-resource rows asserted
  # against `dig(:charge, :charge_model)` of a flat charge and compared everything with nil.
  FIELD_PATHS = {
  ...
  def read(payload, field)
    return payload[field.to_sym] if payload.is_a?(Hash) && payload.key?(field.to_sym)
