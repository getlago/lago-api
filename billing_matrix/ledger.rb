  ...
  class Ledger
    ...
    def self.call(argv)
      ...
      new(
        ...
        entries: YAML.safe_load(File.read(options[:ledger]), permitted_classes: [Date]) || [],
        today: options[:today] ? Date.parse(options[:today]) : Date.today
      ).run(options)
    ...
    def self.parse(argv)
      ...
      OptionParser.new do |o|
        o.on("--results PATH") { options[:results] = _1 }
        o.on("--ledger PATH") { options[:ledger] = _1 }
        o.on("--format FMT", %w[text slack]) { options[:format] = _1 }
        o.on("--apply", "rewrite the ledger in place") { options[:apply] = true }
        o.on("--pr-url URL", "link included in the report") { options[:pr_url] = _1 }
        o.on("--today DATE", "override today, for testing") { options[:today] = _1 }
    ...
    def rows
      @results.fetch("rows", []).reject { _1["area"] == "canary" }
    end

    def compute
      by_id = @entries.to_h { [_1["id"], _1] }
      newly_failed = []
      fixed = []
      still_failing = []
      returned = []
      ...
      rows.each do |row|
        ...
        in ["failed", "fixed"]
          ...
          kept << entry.merge("status" => "failed", "last_seen" => @stamp).tap { _1.delete("fixed_on") }
          returned << entry
        in ["passed", "failed"]
      ...
      {
        entries: kept.sort_by { _1["id"] },
        newly_failed: newly_failed,
        fixed: fixed,
        returned: returned,
        still_failing: still_failing,
        errored: rows.select { _1["verdict"] == "errored" }
      }
    end
    ...
    def warn_about_orphans(orphans)
      ...
      orphans.first(10).each { warn "  - #{_1["id"]}" }
    end
    ...
    def stringify(entry)
      entry.transform_values { _1.is_a?(Date) ? _1.to_s : _1 }
    end
    ...
    def text(diff)
      ...
      section(lines, "newly failing", diff[:newly_failed]) { "#{_1["id"]}\n      #{_1["note"]}" }
      section(lines, "failing again", diff[:returned]) { "#{_1["id"]} (first seen #{_1["first_seen"]})" }
      section(lines, "fixed", diff[:fixed]) { "#{_1["id"]} (failed since #{_1["first_seen"]})" }
      section(lines, "errored", diff[:errored]) { "#{_1["id"]}\n      #{_1["error"]}" }
      unless diff[:still_failing].empty?
        oldest = diff[:still_failing].map { _1["first_seen"] }.min
    ...
    def section(lines, label, items)
      ...
      items.each { lines << "    #{yield(_1)}" }
      lines << ""
    end

    def slack(diff, options)
      parts = ["*Billing matrix* · #{@today.strftime("%-d %b %Y")}"]
      parts << bullets(":red_circle: *#{diff[:newly_failed].size} newly failing*", diff[:newly_failed]) { "`#{_1["id"]}`\n   #{_1["note"]}" }
      parts << bullets(":arrows_counterclockwise: *#{diff[:returned].size} failing again*", diff[:returned]) { "`#{_1["id"]}` — first seen #{_1["first_seen"]}" }
      parts << bullets(":white_check_mark: *#{diff[:fixed].size} fixed*", diff[:fixed]) { "`#{_1["id"]}` — had failed since #{_1["first_seen"]}" }
      parts << bullets(":warning: *#{diff[:errored].size} errored*", diff[:errored]) { "`#{_1["id"]}`\n   #{_1["error"]}" }
      unless diff[:still_failing].empty?
        parts << "_#{diff[:still_failing].size} still failing, already reported (oldest #{diff[:still_failing].map { _1["first_seen"] }.min})_"
      end
      parts << counts
    ...
    def bullets(heading, items)
      ...
      ([heading] + items.map { "• #{yield(_1)}" }).join("\n")
    end
    ...
    def counts
      tally = rows.group_by { _1["verdict"] }.transform_values(&:size)
