...
- `LAGO_RISINGWAVE_USAGE_ENABLED` — when true, current-usage aggregation for eligible charges (count/sum, in arrears, non-prorated, non-recurring, no expression) is served by summing the RisingWave-fed 15-minute usage buckets (ClickHouse `usage_buckets_15m`) over the Rails-computed charges window instead of querying the events store, with automatic fallback when no buckets cover the window. The same flag lets `Events::BillingPeriodFilterService` resolve those charges' charge/filter set from the buckets instead of aggregating every event of the period in `events_enriched_expanded`. Example: `LAGO_RISINGWAVE_USAGE_ENABLED=true`
- `LAGO_RISINGWAVE_USAGE_PARITY_CHECK_ENABLED` — when true, an hourly clock job (`Clock::RealtimeUsageParityCheckJob`) compares the usage buckets against the events-store aggregation and logs mismatches. Example: `LAGO_RISINGWAVE_USAGE_PARITY_CHECK_ENABLED=true`
- `LAGO_KAFKA_WALLET_REFRESH_TRIGGERS_TOPIC` — when set, Karafka consumes this topic (produced by the realtime usage pipeline, keyed by organization+customer) and refreshes the customer's wallets inline on usage changes, replacing the awaiting-refresh flag + clock sweep as the primary trigger. Example: `LAGO_KAFKA_WALLET_REFRESH_TRIGGERS_TOPIC=wallet_refresh_triggers`
