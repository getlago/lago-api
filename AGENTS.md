...
- `LAGO_KAFKA_WALLET_REFRESH_TRIGGERS_TOPIC` — Kafka topic carrying one wallet refresh trigger per enriched count/sum event, keyed by `(organization_id, customer_id)`. When blank the topic is not routed and `WalletRefreshTriggersConsumer` never runs, leaving wallet refresh entirely to the five-minute `Clock::RefreshWalletsOngoingBalanceJob` sweep. Example: `LAGO_KAFKA_WALLET_REFRESH_TRIGGERS_TOPIC=wallet_refresh_triggers`
