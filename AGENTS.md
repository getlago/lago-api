...
- `LAGO_RISINGWAVE_USAGE_ENABLED` — when true, allows current usage to be served from the pre-aggregated ClickHouse usage buckets. The value is cast as a boolean, so `false` or `0` disables serving. Deployment-wide kill switch: serving also requires `LAGO_CLICKHOUSE_ENABLED`, the organization reading the ClickHouse events store, and the per-organization `realtime_usage` feature flag. Example: `LAGO_RISINGWAVE_USAGE_ENABLED=true`
