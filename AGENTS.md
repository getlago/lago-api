...
- `LAGO_RISINGWAVE_USAGE_ENABLED` — when present, allows current usage to be served from the pre-aggregated ClickHouse usage buckets. Deployment-wide kill switch: serving also requires `LAGO_CLICKHOUSE_ENABLED` and the per-organization `realtime_usage` feature flag. Example: `LAGO_RISINGWAVE_USAGE_ENABLED=true`
