...
- `LAGO_WALLET_MAX_AUTOMATIC_TOP_UPS_PER_DAY`: how many automatic threshold top-ups a wallet may make in a rolling 24 hours before the rule refuses and reports. Defaults to 25, which nothing normal reaches. It exists so the ceiling can be raised without a release if it ever refuses a legitimately busy wallet. Anything that is not a positive whole number, including a blank value, falls back to the default rather than to zero, because a ceiling of zero would refuse every top-up. Example: `LAGO_WALLET_MAX_AUTOMATIC_TOP_UPS_PER_DAY=25`
