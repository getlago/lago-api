...
- `LAGO_BILLING_PERIODS_REFRESH_INTERVAL_SECONDS` — interval of the clock job that rolls `subscription_billing_periods` once a period ends. The periods are maintained by the subscription lifecycle services; this is the reconciliation net. Defaults to 5 minutes. Example: `LAGO_BILLING_PERIODS_REFRESH_INTERVAL_SECONDS=300`
