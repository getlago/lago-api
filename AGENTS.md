...
- `LAGO_SMTP_AUTHENTICATION` — SMTP authentication method, one of `login` (default), `plain`, `cram_md5` or `xoauth2`. An empty value, `none` or `disabled` turn authentication off and drop `LAGO_SMTP_USERNAME`/`LAGO_SMTP_PASSWORD` from the mailer settings, which is required because net-smtp otherwise authenticates with PLAIN as soon as a username is present; use it only for a trusted self-hosted relay. Any other value makes every delivery fail — `rails lago:diagnostics` flags it. Example: `LAGO_SMTP_AUTHENTICATION=none`
