...
- `LAGO_RISINGWAVE_USAGE_ENABLED` — when true, current-usage aggregation for eligible charges (count/sum, in arrears, non-prorated, non-recurring, no expression) is served from the RisingWave-fed `usage_realtime_projections` table instead of the events store, with automatic fallback when no row matches. Example: `LAGO_RISINGWAVE_USAGE_ENABLED=true`
- `LAGO_RISINGWAVE_USAGE_PARITY_CHECK_ENABLED` — when true, an hourly clock job (`Clock::UsageProjectionsParityCheckJob`) compares projections against the events-store aggregation and logs mismatches. Example: `LAGO_RISINGWAVE_USAGE_PARITY_CHECK_ENABLED=true`
