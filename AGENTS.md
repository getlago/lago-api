...
- `LAGO_KAFKA_REALTIME_USAGE_TRIGGERS_TOPIC` — Kafka topic carrying one realtime usage trigger per enriched count/sum event, keyed by `(organization_id, customer_id)`. Shared by every reaction to freshly ingested usage, each of which consumes it under its own consumer group; wallet refresh (`WalletRefreshConsumer`) is the only one today. When blank the topic is not routed and no reaction runs, leaving wallet refresh entirely to the five-minute `Clock::RefreshWalletsOngoingBalanceJob` sweep. Example: `LAGO_KAFKA_REALTIME_USAGE_TRIGGERS_TOPIC=realtime_usage_triggers`
- `LAGO_KARAFKA_CONCURRENCY` — number of threads the Karafka process uses to consume partitions in parallel, shared by every consumer group of the process. Defaults to 5. Wallet refreshes run inline in `WalletRefreshConsumer`, so this and the trigger topic's partition count are what set their throughput. Example: `LAGO_KARAFKA_CONCURRENCY=10`
