...
# A selection over ~10 rows has died before as exit 137 with no output: the container is unlimited
# but the 8GB Docker VM is not, and one long run leaves ~300MB of headroom (F94). Count the rows the
# -e filters match (case-insensitive, same substring semantics rspec applies) and warn, don't block.
matrix="$(cd "$(dirname "$0")" && pwd)/../../spec/scenarios/golden/matrix"
if echo "$*" | grep -q "rspec spec/scenarios/golden" && [ -d "$matrix" ]; then
  rows=0 prev="" filtered=""
  for arg in "$@"; do
    if [ "$prev" = "-e" ]; then
      filtered=yes
      rows=$((rows + $(grep -h "^- id: " "$matrix"/*.yml | grep -icF -- "$arg" || true)))
    fi
    prev="$arg"
  done
  [ -z "$filtered" ] && rows=$(grep -h "^- id: " "$matrix"/*.yml | wc -l | tr -d " ")
  if [ "$rows" -gt 10 ]; then
    echo "dev/golden/run.sh: selection matches $rows rows; >10-row runs have died as exit 137 — the VM OOM killer (F94)." >&2
  fi
fi
