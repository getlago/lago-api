      ...
      class FiltersController < Api::BaseController
        ...
        def index
          result = ::ProductItemFiltersQuery.call(
            ...
            filters: {product_item_id: product_item.id}
          )
          ...
          if result.success?
            render(
              json: ::CollectionSerializer.new(
                ...
                meta: pagination_metadata(result.product_item_filters)
              )
            )
          else
        ...
        def destroy
          values = product_item_filter.values.to_a
          result = ::ProductItemFilters::DestroyService.call(product_item_filter:)

          if result.success?
            render_filter(result.product_item_filter, values:)
          else
            render_error_response(result)
          end
        end
        ...
        def find_product_item
          @product_item = current_organization.product_items.find_by(code: params[:product_item_code])

          not_found_error(resource: "product_item") unless product_item
        end

        def find_product_item_filter
          @product_item_filter = product_item.filters.find_by(code: params[:code])
        ...
        def render_filter(filter, values: nil)
          render(json: ::V1::ProductItemFilterSerializer.new(filter, root_name: "filter", values:))
        end
