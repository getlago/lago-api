  ...
  module V1
    class QuotesController < Api::BaseController
      ...
      def index
        ...
        if result.success?
          # QuoteVersionSerializer resolves the billing entity of every embedded version. A version
          # naming none falls back to the subscription it amends, which resolves its own fallback to
          # the customer, so the whole chain is preloaded rather than walked per row.
          quotes = result.quotes.includes(
            {customer: :billing_entity},
            {current_version: :billing_entity},
            {subscription: [:billing_entity, {customer: :billing_entity}]}
          )
