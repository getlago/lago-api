...
module Types
  module Quotes
    class Object < Types::BaseObject
      ...
      def customer
        dataloader
          .with(Sources::ActiveRecord, Customer)
          .load(object.customer_id)
      end

      def organization
        dataloader
          .with(Sources::ActiveRecord, Organization)
          .load(object.organization_id)
      end

      def owners
        dataloader
          .with(Sources::OwnersForQuote, context[:current_organization])
          .load(object.id)
      end
    end
  end
