...
module Charges
  module ChargeModels
    class PackageGroupService < Charges::ChargeModels::BaseService
      ...
      delegate :charge_package_group, to: :charge
      delegate :billable_metric_id, to: :charge
      
      def compute_amount
        return 0 if paid_units.negative?

        if !charge_package_group&.available_group_usage.present?
          reset_available_group_usage
          group_count = initial_group_units
          charge_package_group.update(current_package_count: group_count)
        ...
        if added_group_units > 0
          ...
          group_count = charge_package_group.current_package_count + added_group_units
      ...
      def per_group_package_unit_amount
        @per_group_package_unit_amount ||= BigDecimal(charge_package_group.properties['amount'])
      end
      ...
      def current_package_available_usage
        @current_package_available_usage ||= BigDecimal(charge_package_group.available_group_usage[billable_metric_id])
      end

      def reset_available_group_usage(amount = nil)
        available_group_usage = {}
        charge_package_group.charge.each do |child_charge|
        ...
        charge_package_group.update(available_group_usage:)
      end
    end
  end
end
