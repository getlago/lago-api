...
module Fees
  class ApplyProviderTaxesService < BaseService
    ...
    def call
      ...
      # NOTE: The provider prices the line as a whole, so its jurisdictions are split out of that
      #       figure. Taxing each one on its own would leave them adding up to something the
      #       provider never returned. A response carrying no total for the line falls back to
      #       the sum of its parts rather than booking no tax at all.
      breakdown_amounts = fee_taxes.tax_breakdown.map { |tax| tax.tax_amount || 0 }
      amounts = Integrations::Aggregator::Taxes::Allocation.call(
        fee_taxes.tax_amount_cents || breakdown_amounts.sum,
        breakdown_amounts
      )

      fee_taxes.tax_breakdown.each_with_index do |tax, index|
        ...
        applied_tax.amount_cents = amounts[index]
      ...
      fee.taxes_amount_cents = amounts.sum
