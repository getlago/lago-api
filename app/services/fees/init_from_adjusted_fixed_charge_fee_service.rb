...
module Fees
  class InitFromAdjustedFixedChargeFeeService < ::BaseService
    ...
    def amount_result
      ...
      @amount_result = ChargeModels::Factory
        .new_instance(
          chargeable: ChargeModels::ChargeableData.from_fixed_charge(fixed_charge).with(properties:),
          aggregation_result:
        )
        .apply
    end
    ...
    def fixed_charge
      return adjusted_fee.fixed_charge if adjusted_fee.fixed_charge
      return adjusted_fee.fixed_charge_with_discarded if invoice.voided_invoice_id.present?
