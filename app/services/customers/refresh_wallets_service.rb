...
module Customers
  class RefreshWalletsService < BaseService
    ...
    def initialize(customer:, include_generating_invoices: false, targeted_wallet_ids: nil)
      @customer = customer
      @include_generating_invoices = include_generating_invoices
      @targeted_wallet_ids = targeted_wallet_ids
    ...
    def call
      ...
      wallets_to_process = @targeted_wallet_ids ? customer.wallets.where(id: @targeted_wallet_ids) : customer.wallets.active.includes(:recurring_transaction_rules)
      ...
      wallets_to_process.update_all(last_ongoing_balance_sync_at: Time.current) if @targeted_wallets.present?
