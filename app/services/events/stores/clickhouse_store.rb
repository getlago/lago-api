...
module Events
  module Stores
    class ClickhouseStore < BaseStore
      ...
      # 2. `deduplicated_events_sql` joins to events_enriched to fetch all the requested columns
      #    (in particular the `properties`) only for the matching rows, and then collapses
      #    enriched_at ties via `any()` to ensure exactly one row per dedup key.
      ...
      def deduplicated_events_sql(from_datetime:, to_datetime:, deduplicated_columns: [])
        ...
        aggregated_columns = columns.uniq.map { "any(e.#{it}) AS #{it}" }
        selected_columns = (DEDUP_KEY_COLUMNS.map { "e.#{it}" } + aggregated_columns).join(", ")
        join_conditions = (DEDUP_KEY_COLUMNS.map { "e.#{it} = l.#{it}" } + ["e.enriched_at = l.max_enriched_at"]).join(" AND ")

        <<~SQL.squish
          SELECT #{selected_columns}
          FROM events_enriched AS e
          INNER JOIN latest_enriched AS l ON #{join_conditions}
          WHERE #{deduplicated_events_where_sql(from_datetime:, to_datetime:, alias_prefix: "e")}
          GROUP BY #{DEDUP_KEY_COLUMNS.map { "e.#{it}" }.join(", ")}
        SQL
      end
