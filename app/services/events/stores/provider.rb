...
module Events
  module Stores
    ...
    class Provider
      def initialize(organization:, context:, boundaries:)
        @organization = organization
        @context = context
        @boundaries = boundaries
      end

      attr_reader :context, :boundaries

      # A store minted here answers for the provider's event context and window only. An
      ...
      def scoped_to?(context:, boundaries:)
        self.context == context && self.boundaries == boundaries
      end

      # NOTE: never share the instance between metered items, the aggregators write their
      #       own state (aggregation_property, numeric_property, use_from_boundary) into it.
      def store_for(metered_item:, filters: {})
        plain_store(code: metered_item.billable_metric.code, filters:)
      end

      def plain_store(code: nil, filters: {})
        store_class.new(code:, context:, boundaries:, filters:, deduplicate:)
      end
