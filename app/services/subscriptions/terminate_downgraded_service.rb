...
module Subscriptions
  class TerminateDowngradedService < BaseService
    ...
    def bill_subscriptions
      ...
      # Ignore next subscription since there can't be events
      BillNonInvoiceableFeesJob.perform_later([subscription], rotation_date)
    end
    ...
    def billable_subscriptions
      ...
      if next_subscription.plan.pay_in_advance? || next_subscription.fixed_charges.pay_in_advance.any?
        [subscription, next_subscription]
      else
        [subscription]
      end
