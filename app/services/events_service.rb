...
class EventsService < BaseService
  def validate_params(params:)
    ...
    mandatory_subscription_augments = %i[subscription_id customer_id]

    missing_arguments = mandatory_augments.select { |arg| params[arg].blank? }
    missing_subscription_arguments = mandatory_subscription_augments.select { |arg| params[arg].blank? }
  ...
  def create(organization:, params:, timestamp:, metadata:)
    unless current_customer(organization.id, params[:customer_id], params[:subscription_id])
    ...
    event.subscription_id = params[:subscription_id] || attached_subscriptions.first
  ...
  def batch_create(organization:, params:, timestamp:, metadata:)
    ...
    unless current_customer(organization.id, params[:customer_id], params[:subscription_ids]&.first)
      return invalid_customer_error(organization, params)
    end
  ...
  def current_customer(organization_id = nil, customer_id = nil, subscription_id = nil)
    return @current_customer if defined? @current_customer

    @current_customer = if subscription_id
                          Subscription.find_by(id: subscription_id).customer
                        else
                          Customer.find_by(customer_id: customer_id, organization_id: organization_id)
                        end
  end
  ...
  def blank_subscription_error(organization, params)
    result.fail!('missing_argument', 'subscription does not exist')
    send_webhook_notice(organization, params)
