...
module CreditNotes
  class CreateFromTermination < BaseService
    ...
    def creditable_used_amount
      base_creditable_used_amount = base_subscription_used_amount - last_subscription_fee.credit_note_items.sum(:amount_cents)

      adjust_for_coupon_and_taxes(base_creditable_used_amount)
    end

    def paid_amount_prorated_to_subscription
      invoice_amount = last_subscription_fee.invoice.total_amount_cents

      return 0 if invoice_amount.zero?

      fee_amount = last_subscription_fee.precise_total_amount_cents
