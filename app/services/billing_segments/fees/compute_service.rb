...
module BillingSegments
  module Fees
    # Orchestrates billing-segment fee creation. It calls the pricing and amount services,
    # then decorates the unsaved product fee and optional linked true-up fee. The
    # scheduler stores the exact pricing records on the segment, so processing retries
    ...
    #
    # Proration: the scheduler stores the fee ratio on the billing segment. It is 1 for
    ...
    class ComputeService < BaseService
      ...
      delegate :currency, :rate, :rate_card_rate, :rate_override, :subscription_rate_card, to: :billing_segment
      ...
      def fee
        @fee ||= Fee.new(
          ...
          properties: {
            "from_datetime" => billing_segment.period_from.iso8601(3),
            "to_datetime" => billing_segment.period_to.iso8601(3)
          }
        )
      end
      ...
      # A pay-in-advance increase: the period was already invoiced, so this segment owes only
      ...
      def watermark
        @watermark ||= BillingSegments::ResolveWatermarkService.call!(billing_segment:).units
      end

      def incremental_amount
        @incremental_amount ||= begin
          delta = BillingSegments::Fees::AdvanceDeltaService.call!(
            billing_segment:,
            units:,
          ...
          BillingSegments::Fees::AmountsService::Amount.new(
            amount_cents:,
      ...
      def fee_amounts
        @fee_amounts ||= BillingSegments::Fees::AmountsService.call!(
          billing_segment:,
          charge_model_result:,
      ...
      # Resolved over the segment's own window rather than read off the card, because a units
      ...
      def resolved_units
        @resolved_units ||= SubscriptionRateCards::ResolveUnitsService.call!(
          ...
          from: billing_segment.period_from,
          to: billing_segment.period_to
        )
      end
      ...
      def aggregation_result
        prorated_units = billable_units * billing_segment.proration_ratio
