...
module BillingSegments
  module Fees
    class AmountsService < BaseService
      ...
      def initialize(billing_segment:, charge_model_result:, currency:, units:)
      ...
      attr_reader :billing_segment, :charge_model_result, :currency, :units
      ...
      def true_up_amount_units
        if billing_segment.pricing_unit
          return true_up_currency_amount / billing_segment.pricing_unit_conversion_rate
        end
      ...
      def minimum_amount_cents
        (billing_segment.min_amount_cents * billing_segment.proration_ratio).round
      end
      ...
      class Amounts
        def self.build(billing_segment:, currency:)
          if billing_segment.pricing_unit
            return WithPricingUnit.new(billing_segment:, currency:)
          end
      ...
      class WithPricingUnit < Amounts
        def initialize(billing_segment:, currency:)
          @billing_segment = billing_segment
          super(currency:)
        end
