...
module BillingSegments
  # Consumer lane, scoped to ONE customer. Loads the customer's pending segments, groups
  ...
  # double-invoicing the same segments, and it makes the run see the COMPLETE set (a
  ...
  class ProcessService < BaseService
    ...
    def call
      ...
      customer.with_advisory_lock("billing_segment_process_customer_#{customer.id}") do
        pending_segments.group_by { |segment| invoice_key(segment) }.each_value do |segments|
          result.invoices << build_invoice(segments)
        end
        ...
        # invoices, so it doesn't re-create — the segments are already `done`).
    ...
    # segment onto its own invoice.
    def invoice_key(segment)
      subscription = segment.subscription
      [
        billing_date(segment),
        subscription.consolidate_invoice ? :shared : segment.id,
        segment.currency,
    ...
    def build_invoice(segments)
      subscriptions = segments.map(&:subscription).uniq
      ...
      ActiveRecord::Base.transaction do
        invoice_result = Invoices::CreateGeneratingService.call(
          ...
          create_invoice_subscriptions(generating_invoice, segments)
        end
        invoice_result.raise_if_error!
        invoice = invoice_result.invoice

        segments.flat_map { |segment| computed_fees(segment) }.each do |fee|
          fee.invoice = invoice
    ...
    # The billing_segment already carries the exact period per item, so the new engine owns
    # its invoice_subscription link directly — one per subscription, boundaries spanning that
    # subscription's segments. This replaces the legacy CreateInvoiceSubscriptionService, which
    ...
    def create_invoice_subscriptions(invoice, segments)
      segments.group_by(&:subscription).each do |subscription, subscription_segments|
        period_from = subscription_segments.map(&:period_from).min
        period_to = subscription_segments.map(&:period_to).max
    ...
    # Finalize every still-generating invoice this customer's segments produced — this run's
