...
module Invoices
  module Payments
    class CashfreeService < BaseService
      ...
      def create
        Rails.logger.debug "invoice.total_amount_cents 0: #{invoice.total_amount_cents}"

        result.invoice = invoice
        return result unless should_process_payment?

        Rails.logger.debug "invoice.total_amount_cents 1: #{invoice.total_amount_cents.positive?}"
        ...
        Rails.logger.debug "invoice.total_amount_cents 2: #{invoice.total_amount_cents}"
      ...
      def update_payment_status(provider_payment_id:, status:, metadata: {})
        payment = if metadata[:payment_type] == 'one-time'
          create_payment(provider_payment_id:)
        else
      ...
      def create_payment(provider_payment_id:)
        @invoice = Invoice.find_by(id: provider_payment_id)
      ...
      def payment_url_params
        # TODO Add settings_accessors if required
        {
          customer_details: {
