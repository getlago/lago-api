...
module Auth
  module EntraId
    class BaseService < BaseService
      ...
      # NOTE: Entra ID only returns the `email` claim when the account has a mail
      #       attribute and the claim is configured, so we fall back to
      ...
      def check_userinfo(email)
        ...
        raise ValidationError, "entra_id_userinfo_error" unless response_email&.casecmp?(email)
