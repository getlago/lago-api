  ...
  class PopulatePaymentsWithCustomerId < ApplicationJob
    ...
    def perform(batch_number = 1)
      ...
      Payment.where(payable_type: "Invoice", customer_id: nil)
        .where("payable_id IN (SELECT id FROM invoices WHERE customer_id IS NOT NULL)")
        .limit(BATCH_SIZE)
      ...
      Payment.where(payable_type: "PaymentRequest", customer_id: nil)
        .where("payable_id IN (SELECT id FROM payment_requests WHERE customer_id IS NOT NULL)")
        .limit(BATCH_SIZE)
