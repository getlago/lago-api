...
module Analytics
  class DraftInvoicesCollection < Base
    ...
    class << self
      def query(organization_id, **args)
        ...
        sql = <<~SQL.squish
          ...
          FROM all_months am
          LEFT JOIN draft_invoices dis ON dis.month = am.month AND dis.currency IS NOT NULL
          WHERE am.month <= DATE_TRUNC('month', CURRENT_DATE)
          #{and_months_sql}
          #{and_currency_sql}
          ORDER BY am.month, dis.currency;
        SQL
      ...
      def expire_cache_for_customer(organization_id, external_customer_id)
        Rails.cache.delete_matched(
          "draft-invoices-collection/#{Date.current.strftime("%Y-%m-%d")}/#{organization_id}*#{external_customer_id}*"
        )
      end
    end
  end
