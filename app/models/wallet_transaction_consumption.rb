...
class WalletTransactionConsumption < ApplicationRecord
  ...
  belongs_to :inbound_wallet_transaction, class_name: "WalletTransaction"
  belongs_to :outbound_wallet_transaction, class_name: "WalletTransaction"

  validates :amount_cents_consumed, numericality: {greater_than: 0}
  validate :inbound_transaction_must_be_inbound
  validate :outbound_transaction_must_be_outbound

  def credits_consumed
    amount_cents_consumed / (inbound_wallet_transaction.wallet.rate_amount * 100.0)
  end
  ...
  def inbound_transaction_must_be_inbound
    ...
    errors.add(:inbound_wallet_transaction, :invalid_transaction_type)
  end
  ...
  def outbound_transaction_must_be_outbound
    ...
    errors.add(:outbound_wallet_transaction, :invalid_transaction_type)
  end
end
...
#  amount_cents_consumed          :bigint           not null
